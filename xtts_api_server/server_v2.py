...
async def tts_ulaw(
    ...
    """
    Generate an audio stream from the provided text using the TTS model, 
    convert the audio to u-law format with 8kHz sample rate, and stream it as Base64.
    """
    try:
        if XTTS.model_source != "local":
            raise HTTPException(status_code=400, detail="HTTP Streaming is only supported for local models.")
        ...
        # Return the streaming response with Base64-encoded u-law audio
